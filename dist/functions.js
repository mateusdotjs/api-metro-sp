...
export function getOne(html, idReq) {
    let dado = null;
    const $ = cheerio.load(html);
    const listaLinhas = $("ol").find("li");
    for (let i = 0; i < listaLinhas.length; i++) {
...
function formatAll(dados) {
    let linhas = dados.map((linha) => {
        ...
        let status = linha.status === "Circulação de Trens"
            ? "Velocidade reduzida"
            : linha.status;
        return {
            id: linha.id,
...
function formatOne(dado) {
    ...
    let status = dado.status === "Circulação de Trens" ? "Velocidade reduzida" : dado.status;
    return {
        id: dado.id,
        titulo,
        status,
